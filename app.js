...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCV() {
        ...
        if (state.theme === 'classic') {
            html = `<div class="theme-classic"><header class="cv-header">${photoHTML}<div class="cv-name">${esc(fullName)}</div>${p.jobTitle?`<div style="font-weight:500;color:#4B5563;margin-top:4px">${esc(p.jobTitle)}</div>`:''}${contactHTML}</header>${buildSections(exps, edus)}</div>`;
        } else if (state.theme === 'modern') {
            html = `<div class="theme-modern"><aside class="cv-sidebar" style="background:${state.color}">${photoHTML}<div class="cv-name">${esc(fullName)}</div>${p.jobTitle?`<div style="opacity:0.8;margin-bottom:24px;font-size:13px;text-align:center">${esc(p.jobTitle)}</div>`:''}
            <div class="cv-section"><div class="cv-section-title">Contact</div>${p.email?`<div class="cv-contact-item">✉ ${esc(p.email)}</div>`:''}${p.phone?`<div class="cv-contact-item">📞 ${esc(p.phone)}</div>`:''}${p.location?`<div class="cv-contact-item">📍 ${esc(p.location)}</div>`:''}</div>
            ${state.skills.technical.length?`<div class="cv-section"><div class="cv-section-title">Tech</div><div style="display:flex;flex-wrap:wrap;gap:6px">${state.skills.technical.map(s=>`<span style="background:rgba(255,255,255,0.2);border:1px solid rgba(255,255,255,0.3);color:white;font-size:11px;padding:3px 8px;border-radius:4px">${esc(s)}</span>`).join('')}</div></div>`:''}
            ...
            ${state.languages.length?`<div class="cv-section"><div class="cv-section-title">Langues</div>${state.languages.map(l=>`<div style="margin-bottom:6px;font-size:12px"><strong>${esc(l.main)}</strong><br><span style="opacity:0.7">${esc(l.sub)}</span></div>`).join('')}</div>`:''}</aside><main class="cv-main">${buildSections(exps, edus, true)}</main></div>`;
        } else {
            html = `<div class="theme-elegant"><header class="cv-header">${photoHTML}<div><div class="cv-name">${esc(fullName)}</div>${p.jobTitle?`<div style="font-style:normal;margin-bottom:8px">${esc(p.jobTitle)}</div>`:''}<div style="display:flex;gap:16px;font-size:13px;color:#6B7280;font-style:normal;font-family:'Inter',sans-serif;flex-wrap:wrap">${p.email?`<span>✉ ${esc(p.email)}</span>`:''}${p.phone?`<span>📞 ${esc(p.phone)}</span>`:''}${p.location?`<span>📍 ${esc(p.location)}</span>`:''}</div></div></header>${buildSections(exps, edus)}</div>`;
        }
    ...
    function buildSections(exps, edus, modern = false) {
        let h = '';
        if (state.profile) h += `<section class="cv-section"><div class="cv-section-title">Profil</div><div class="cv-text">${esc(state.profile).replace(/\n/g, '<br>')}</div></section>`;
        ...
        if (!modern) {
            ...
            if (state.languages.length) h += `<section class="cv-section"><div class="cv-section-title">Langues</div>${state.languages.map(l=>`<div style="display:flex;justify-content:space-between;padding:4px 0;font-size:13px"><span style="font-weight:600">${esc(l.main)}</span><span style="color:#666">${esc(l.sub)}</span></div>`).join('')}</section>`;
        }
        return h;
    }
